...
class EnhancedSchedulerPlugin(Star):
    def __init__(self, context: Context, config: Optional[dict] = None):
        ...
        self.legacy_llm_log_file = os.path.join(self.data_dir, LEGACY_LLM_LOG_FILE_NAME)
    ...
    async def _api_get_llm_logs(self):
        ...
        offset = max(0, offset)
        try:
            total, entries = await asyncio.to_thread(self._llm_log_page_io, offset, limit)
    ...
    async def _api_get_llm_log_detail(self):
        ...
        try:
            ...
            result = await asyncio.to_thread(self._read_llm_record_io, name)
            if result is None:
...
# ─────────────────────────────────────────────────────────────────────
# LLM 日志存储辅助（图片抽离、元数据、文件名）
# ─────────────────────────────────────────────────────────────────────

# 记录中的 base64 图片数据 URL
